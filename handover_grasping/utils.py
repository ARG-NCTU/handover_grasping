...
__all__ = ['Image_table', 'get_grasp_line', 'get_affordancemap', 'get_model', 'get_pcd_right', 'get_pcd_left',
           'get_view', 'vis', 'get_line_len', 'width_detect', 'get_angle', 'label_zeng2HA']
...
def get_angle(point_1, point_2):
    """get line angle to horizon

    This function compute surface wline angle to horizon and remap to id 0~3.

    Args:
        point_1(list):[p1_x, p1_y]
        point_1(list):[p2_x, p2_y]
        theta : degrees (0, 90, +-45)
...
def label_zeng2HA():
    data_path = '/home/arg/handover_grasping/data/parallel-jaw-grasping-dataset'
    os.mkdir(data_path + '/HA_label')
    name = []
    f = open(data_path+'/train-split.txt', "r")
    for _, line in enumerate(f):
        name.append(line.replace("\n", ""))

    for n in name:
        g = np.loadtxt(data_path+'/label/'+n+'.good.txt')
        label = np.zeros((4,224,224))

        id = int(round(g.shape[0]/2))
        ...
        if g.ndim == 2:
            point_list = g[id]
            flag = 1
        elif g.ndim == 1 and g.shape[0] != 0:
            point_list = g
            flag = 1

        if flag != 0:
            p1 = (int(point_list[0]/1.42857),int(point_list[1]))
            p2 = (int(point_list[2]/1.42857),int(point_list[3]))
            theta = get_angle(p2, p1)
            cv2.line(label[theta],p1,p2,255,2)

        np.save(data_path+'/HA_label/'+n+'.npy', label)